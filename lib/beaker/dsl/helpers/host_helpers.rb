  ...
  module DSL
    module Helpers
      ...
      module HostHelpers
        ...
        # Move a local file or directory to a remote host using docker exec
        #
        # @return [Result] Returns the result of the rsync operation
        def docker_cp_to host, from_path, to_path, opts = {}
          block_on host do | host |
            @result = host.do_docker_cp_to(from_path, to_path, opts)
            @result.log logger
            @result
          end
        end
        ...
        def create_remote_file(hosts, file_path, file_content, opts = {})
          Tempfile.open 'beaker' do |tempfile|
            ...
            case opts[:protocol]
              when 'docker'
                docker_cp_to hosts, tempfile.path, file_path, opts
              when 'scp'
